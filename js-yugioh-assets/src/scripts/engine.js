...
async function removeAllCardsImages() {
  const computerCards = state.playerSides.computerBOX;
  const playerCards = state.playerSides.player1BOX;

  computerCards.querySelectorAll("img").forEach((img) => img.remove());
  playerCards.querySelectorAll("img").forEach((img) => img.remove());
}
